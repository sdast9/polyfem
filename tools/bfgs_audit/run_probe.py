"""Build the BFGS audit against an existing macOS Makefiles PolyFEM build."""
...
def main():
    ...
    flags = {}
    flag_file = build/'_deps/polysolve-build/CMakeFiles/polysolve.dir/flags.make'
    ...
    sources = [source] + [polysolve/'src/polysolve/nonlinear'/p for p in (
        'descent_strategies/BFGS.cpp', 'descent_strategies/LBFGS.cpp',
        'descent_strategies/box_constraints/LBFGSB.cpp')]
    needed = {
    ...
    record = {
        'build_command': cmd, 'run_command': [str(executable)],
        'source_sha256': {str(p): sha(p) for p in sources},
        'library_sha256': {str(p): sha(p) for p in libs},
        'polysolve_commit': subprocess.check_output(
            ['git', '-C', str(polysolve), 'rev-parse', 'HEAD'], text=True).strip(),
        'polysolve_status': subprocess.check_output(
            ['git', '-C', str(polysolve), 'status', '--short'], text=True),
    }
    ...
    with (out/'results.jsonl').open('w') as log:
        try:
            result = subprocess.run([str(executable)], stdout=log,
    ...
    if record['run_exit_code'] != 0:
        raise SystemExit('Probe did not complete; execution evidence was retained.')
