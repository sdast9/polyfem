"""Build/run the RB-02 coefficient characterization against an existing Makefiles build.

Usage: python3 tools/rb02/run_probe.py --build build --output /absolute/evidence
...
source = Path(__file__).with_name('coefficient_probe.cpp').resolve()
(out / source.name).write_bytes(source.read_bytes())
obj, binary = out / 'coefficient_probe.o', out / 'coefficient_probe'
