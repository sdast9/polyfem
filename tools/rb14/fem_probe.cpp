// RB-14 assembled-FEM experiment. No production coefficient policy is installed.
...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		for (const std::string method : {"full", "radius0", "radius1", "radius2", "diagonal", "direction", "current_trim1"})
		{
			std::vector<int> ids;
			auto neighborhood_start = Clock::now();
			if (method.rfind("radius", 0) == 0)
			{
