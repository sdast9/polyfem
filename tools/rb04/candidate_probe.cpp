	...
	try
	{
		...
		for (bool fixed : {false, true})
		{
			...
			for (double eps : {1e-3, 1e-5, 1e-7, 1e-9})
			{
				...
				f.first_derivative(left, gl);
				auto r = sample(f, right);
				f.first_derivative(right, gr);
				rows.push_back({{"epsilon", eps}, {"left", l}, {"right", r}, {"energy_jump", double(r["energy"]) - double(l["energy"])}, {"gradient_jump_norm", (gr - gl).norm()}});
				if (!fixed)
					check(close(l["scales"][0], 70) && close(r["scales"][0], 55), "RB02 feature reproduction");
				f.line_search_end();
			}
			json fd;
			V x = z;
			...
			f.solution_changed(a);
			const double ea = f.value(a);
			f.solution_changed(b);
			const double eb = f.value(b);
			...
			for (int n : {16, 64, 256, 1024})
			{
				...
				for (int side = 0; side < 2; ++side)
					...
					{
						V q = z, g;
						q[4] = .9 + .1 * side + .1 * (i + .5) / n;
						f.solution_changed(q);
						f.first_derivative(q, g);
						integral += g[4] * .1 / n;
					}
				work.push_back({{"panels_per_side", n}, {"gradient_integral", integral}, {"energy_change", eb - ea}, {"energy_minus_integral", eb - ea - integral}});
			}
			f.line_search_end();
			...
			if (fixed)
				check(double(rows.back()["gradient_jump_norm"]) < 1e-4, "fixed limiting gradient continuity");
			else
				check(std::abs(double(rows.back()["energy_jump"])) > 40, "semi nonvanishing energy jump");
		}
		auto m = mesh();
		Probe fixed(m, 1, json::object(), 1, true, 100), semi(m);
		fixed.start(z);
		semi.start(z);
