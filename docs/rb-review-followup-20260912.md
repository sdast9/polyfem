...
Status: **validated within the bounded review scope on the published RB-05 foundation**. The user accepted the review's recommendation to repair its four reproduced defects and correct the RB-03 algebra wording. This corrects those bounded parts of RB-18, RB-19 and RB-21; it does not select a new coefficient or interpolation model.
...
## Validation on the initial foundation
...
Publication: PolySolve [`ee5b296a6`](https://github.com/sdast9/polysolve/commit/ee5b296a690ce225fb34f6f375d27f9ea2a16027) on `sdast9/polysolve:iteration-callback`; PolyFEM's companion pin, implementation, regressions and this record are published together on `sdast9/polyfem:main`. This first validation used IPC `e3c8d3fe6`; the later RB-05 integration below is the final publication foundation. The shared build is not rebuilt; the validated executable is `outputs/rb-review-fixes/20260912T201345Z/build/PolyFEM_bin` in the parent workspace.

## Limits retained

These are targeted implementation repairs, not a proof of engineering accuracy or all floating-point paths. The characteristic scale bounds tolerated energy noise; a decreasing gradient alone still does not prove descent on a nonconvex objective. The extreme `weight * scale` case is a named unsupported-arithmetic error, even when reordering later products could represent its exact energy; general IPC multiplication rescaling is not implemented here. Failed-attempt rollback remains RB-06's contract. RB-05 is preserved as subsequently published; RB-09/RB-10 accuracy work remains open, RB-17 remains retired, and the selected coefficient/interpolation/controller laws are retained. No Teseo or other private scenes were run.

## Integration with published RB-05 (later 2026-09-12)

While this task was validating in isolation, RB-05 was published through PolyFEM `382c4c21e` (implementation `db38cbff2`, automatic-limit/exit-status follow-up `377a83fda`) and IPC `bb7954468`. Separate RB-10 work then appeared in the shared workspace. The earlier status notices about ongoing/uncommitted RB-05 describe the original checkpoint; none of those shared files were edited by this task.

The private fix commit was rebased onto `382c4c21e` and the private IPC checkout advanced to `bb7954468`. The only rebase conflict was the RB-05 plan status row; its published status and the user's automatic-limit decision were kept. The source changes merge without overlap: RB-05 adds diagnostic statistics in a different part of `BarrierContactForm.cpp`. The old build evidence above is retained under `final-validation`; the new build and suite (also including `[resource_containment]`), public smokes, step-1 repeats and updated HDA test are tracked in `rb05-integration/`. This integration is now complete: **65 PolyFEM cases / 4,315 assertions**, the **4-case / 534-assertion PolySolve suite**, all **five smokes** (five saved states each, exits 0, no error lines), all **13 step-1 repeats** (16 timesteps, zero restarts), and the **updated HDA end-to-end test** pass. Default-thread fallback counts are 5–10, single-thread 10 each. The maximum smoke endpoint difference from the updated published controls is `8.9511731360403246e-16`. All four numerical counterexamples are re-confirmed before/after against this foundation; supporting archive hashes remain stable. HDA input hashes and final binary/source hashes are in the committed results JSON's `published_rb05_integration` entry.

Final production commit: [`c93cdd9c52`](https://github.com/sdast9/polyfem/commit/c93cdd9c52c68a52248c56660aaa355218c46eda), on `sdast9/polyfem:main`, with PolySolve `ee5b296a6` and the existing RB-05 IPC pin `bb7954468`. The subsequent documentation commit records this integration evidence. **Shared RB-10 edits and the shared source checkouts/build remain untouched**: those checkouts intentionally remain at their incoming revisions while RB-10 continues. The published fixes and validated executable are in the isolated checkout/build identified above.
