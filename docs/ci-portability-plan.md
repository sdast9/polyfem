...
**Investigation update, 2026-09-20:** the [golden-test implementation plan](golden-unit-test-repair-plan-20260920.md) gives the next model concrete work packages for CI-03–CI-06. Current Linux/macOS Release logs still show four failing groups containing six inputs. New local A/B runs confirm the historical friction-budget cause for both large-mass-ratio scenes, recover all six stretch-cubes references with either existing proxy, and reproduce fresh-process GCP variation with a specific hash-ordering lead. No golden/default/solver repair was applied by this investigation; P4 causality, microstructure's resource profile, and GCP's final reference policy remain to be established. The latest Linux Debug rollback-test segfaults are separate failures and are recorded in the new plan.
