...
**Implementation update, 2026-09-21 (CI-03, [record](ci-03-validation.md)):** the three fixtures of CI-03 are explained entirely by RB-10's `friction_iterations` default (1 → 2): on isolated copies run through the scene harness itself, the before-revision default, the before-revision explicit 1 and the current-revision explicit 1 are one bit-identical number set that authenticates all three (5e-12 / 8e-10 / 3e-13), and the current default, explicit 2 and the before-revision explicit 2 are another that equals both Release lanes' values. The fixtures now state that policy, each has a `<name>-friction-defaults.json` twin running at the current default with a harness-generated reference, and both pass; the data set moved to a writable home, `sdast9/polyfem-data@e6ed5cf` (branch `fable-fixtures`, user decision 2026-09-21), and the harness gained `run_manifest_env` (any manifest against any data directory). **CI-03 is complete**, read natively from [Build run 35617547219](https://github.com/sdast9/polyfem/actions/runs/35617547219) at `0c129dcfb`: Linux Release 378/381 and macOS Release 377/380, the six fixtures authenticated on both, `contact_3d` green on both, the only failures `contact_2d` (CI-06), `standard` (CI-04) and `triangle_data` (CI-05); Windows Release green. (That run also carried a one-line GCC repair of `tests/test_output_kinematics.cpp` from the RBR-01 follow-up, the CI-01 brace pattern, without which no Linux lane built.)
...
**Complete (2026-09-21, [record](ci-03-validation.md); native acceptance run 35617547219: Linux Release 378/381, macOS Release 377/380, `contact_3d` green on both).** Steps 1–4 below were done as written: isolated copies at `91c7ff8ac` and `11cdf476f` through the harness (`run_manifest_env`), the omitted / 1 / 2 comparison with the resolved input and the RB-04 lagging record, the historical settings encoded in the three fixtures, current-defaults twins with harness-generated references checked against both Release lanes' values (≤ 6e-11) and against a budget sweep to the converged lag (2D and the ball: the default is closer than the single solve; 3D: the second lag iterate overshoots by 33 % in `err_h1_semi` — an observation handed to RB-10, not a change). The production defaults are untouched; no existing reference value changed. The data set is now pinned to the fork `sdast9/polyfem-data@e6ed5cf` (branch `fable-fixtures`); CI-06's data change has the same home.
...
10. **IPC Toolkit fork, Linux lanes: `-march=native` objects across runners (from RBR-02, 2026-09-20).** The `Build` workflow restores `actions/cache` + ccache keyed only by OS and configuration while `IPC_TOOLKIT_WITH_SIMD` compiles with `-march=native`; [run 35546941978](https://github.com/sdast9/ipc-toolkit/actions/runs/35546941978) restored the 2026-09-14 Linux Release objects (ccache 350/423 hits) onto a runner without that CPU's instructions and 181 of 296 tests died with SIGILL, none of them touched by the change (`outputs/rbr-02/20260920T202503Z/ci/`). Deleting the two Linux caches and re-dispatching gave a clean rebuild. Key the cache by the runner's CPU flags (or drop `-march=native` in CI); until then a Linux SIGILL storm after a cache hit is an infrastructure artefact, and the caches must be cleared before reading the lane.
