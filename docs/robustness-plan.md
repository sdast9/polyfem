...
| Enabled production resource or AL budgets | RB-05 / RB-07 | Measure overhead/failure behavior and state proposed limits; user selects defaults; opt-in disabled-by-default mechanisms may be tested first. **Resource limits decided 2026-09-12 (RB-05: on by default, automatic).** AL budget: implemented off by default 2026-09-15 (RB-07); the enabled default — `max_passes` and/or `stagnation_window`, with which numbers, and a Houdini control — is still the user's choice; the measured consequences on the public fixtures are in the [record's decision section](rb-07-validation.md#decision-required-production-default) |
...
history in the failure record and the RB-06 rollback on exit. The user has not
selected an enabled default; the decision register row stays open for it.
