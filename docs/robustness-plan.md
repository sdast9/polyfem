...
| RB-12 | Repeatability, provenance and release checks | none for provenance; relevant RB checks for release | [in progress 2026-09-14 — stage 1 (run identity) validated within stated scope and published `1f6f826fa`: every `PolyFEM_bin` run writes `run-manifest.json` (build identity compiled in before every build with the effective PolyFEM/IPC/PolySolve checkouts and declared pins, executable SHA-256, platform/threads, effective input and referenced-file hashes, model description, per-step attempt history, completion); stage 2 characterized—limits documented: single-threaded runs bit-reproducible on the three public fixtures, threaded frictionless endpoints to ≤ 7e-16, threaded friction to 1.7e-4 through a trim decision on an unconverged lag, PF-08's fine-load failure not reproduced (10/10), peak RSS ~125 MB per thread; stage 3 in progress: `--build_info`, CTest `cli_contract` on every platform, always-on test-log artifacts, sccache fix, defaults table, CI classification](rb-12-validation.md) |
