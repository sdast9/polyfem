...
| RB-12 | Repeatability, provenance and release checks | none for provenance; relevant RB checks for release | [in progress 2026-09-14 — stage 1 (run identity) validated within stated scope and published `1f6f826fa`: every `PolyFEM_bin` run writes `run-manifest.json` (build identity compiled in before every build with the effective PolyFEM/IPC/PolySolve checkouts and declared pins, executable SHA-256, platform/threads, effective input and referenced-file hashes, model description, per-step attempt history, completion); stage 2 characterized—limits documented: single-threaded runs bit-reproducible on the three public fixtures, threaded frictionless endpoints to ≤ 7e-16, threaded friction to 1.7e-4 through a trim decision on an unconverged lag, PF-08's fine-load failure not reproduced (10/10), peak RSS ~125 MB per thread; stage 3 in progress: `--build_info`, CTest `cli_contract` on every platform, always-on test-log artifacts, sccache fix, defaults table, CI classification, two Debug-only repairs (`Mesh::create` on geogram volume meshes, obstacle rows of averaged output fields), dependency-fork CI on the maintained branches with the pins following (`4f5acc773`), Houdini `provenance` block (`6f5fcdc`)](rb-12-validation.md) |
...
stage 3 in progress — see [the record](rb-12-validation.md). The manifest's
schema is documented in [scenes/semi-implicit/README.md](../scenes/semi-implicit/README.md#run-manifest-rb-12);
the runners are `tools/rb12/`. Open for the user: the CI scope (required lanes;
the RB-11 Debug/Windows test failures belong to RB-11's follow-up), and whether
the per-thread memory growth measured in stage 2 deserves an item.
