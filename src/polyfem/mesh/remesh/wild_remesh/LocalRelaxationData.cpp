...
namespace polyfem::mesh
{
	...
	void LocalRelaxationData<M>::init_assembler(const State &state)
	{
		...
		assembler->set_materials(local_mesh.body_ids(), state.args["materials"], state.units, state.root_path(), state.material_file_cache());

		mass_matrix_assembler = std::make_shared<assembler::Mass>();
		mass_matrix_assembler->set_size(dim());
		mass_matrix_assembler->set_materials(local_mesh.body_ids(), state.args["materials"], state.units, state.root_path(), state.material_file_cache());
